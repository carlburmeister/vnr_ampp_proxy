...
export class SoundProbeClient extends EventEmitter {
  ...
  public async startNotificationListener(): Promise<boolean> {
    ...
    //this.gvPlatform.on('reconnecting', () => clearInterval(this.subscriptionInterval));
    this.gvPlatform.on('reconnecting', () => clearInterval(this.subscriptionInterval as any));
    
    this.gvPlatform.on('reconnected', this.startAudioMeterSubscriptionAsync.bind(this));
