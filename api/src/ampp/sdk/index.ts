...
import { getIPAddress, getNetmask } from './helpers';
...
const netmask = getNetmask(ipAddress);

const pushNotificationConfig = {
  // General push notification configurations
  retryIntervalAfterLostConnectionMs: 5000,
  maxRetryCountAfterLostConnection: 3,
  searchForLocalpushIntervalMs: 10000,

  // Local Push is a way to send and receive Push Notifications from a Local Push Notification Relay (LPNR)
  // without the need for the notifications to round-trip to an instance of the notifications service in the Cloud.
  // This allows apps to keep working locally even if internet connectivity is lost.
  // YourLocalPushSiteId - enables local push notifications, optional
  localPushSiteId: 'MichalKLocalPush',
  // IpAddress - used to find LPNR, optional
  localIpAddress: ipAddress,
  // Netmask - used to find LPNR, optional
  localSubnetMask: netmask,
};
...
async function subscribeToKeyframes() {
  ...
  const FOLDER_PATH = './keyframes';
  //const FOLDER_PATH = path.join(process.cwd(), 'keyframes');
  ...
  const keyframesClient = new KeyframesClient(platformUrl, apiKey, FOLDER_PATH, pushNotificationConfig);
  await keyframesClient.login();
  console.log('Login Ok');

  console.log('**********Keyframes*************');
  const started = await keyframesClient.startNotificationListener();
  if (!started) {
    console.error('Failed to start notification listener');
    return;
  }
...
async function amppControlSdk() {
  ...
  const ampp = new AmppControl(platformUrl, apiKey, pushNotificationConfig);
  await ampp.login();
  console.log('Login Ok');
  ...
  console.log('**********Workload Names for FlowMonitor*************');
   // Get a List of workloads that are MiniMixers
  var workloadnames = await ampp.listWorkloadNamesForApplicationType('FlowMonitor');
  console.log(workloadnames);
  console.log('***********************************************');
  console.log();
  console.log();
...
async function subscribeToAudioLevels() {
  ...
  audioMeterClient.on('audiometer', (notification: ProbeNotification) => {
    ...
    console.log(
      `id: ${notification.id}, time: ${notification.time}, rms: ${notification.rms}, peak: ${notification.peak}`,
    );
  });
